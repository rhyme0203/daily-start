...
export class DdanziCrawler extends BaseCrawler {
  constructor() {
  ...
  async crawlPosts(): Promise<CrawledPost[]> {
    try {
      ...
      postElements.forEach((element, index) => {
        ...
        try {
          const titleElement = element.querySelector('a[href*="/dogdrip/"]')
      ...
      console.error('Error crawling Ddanzi posts:', error)
      return []
    }
  }
  ...
  async crawlPostContent(postUrl: string): Promise<string> {
    try {
      ...
      console.error('Error crawling Ddanzi post content:', error)
