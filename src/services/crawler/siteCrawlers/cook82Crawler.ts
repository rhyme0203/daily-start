...
export class Cook82Crawler extends BaseCrawler {
  constructor() {
    const config: CrawlerConfig = {
      ...
      listUrl: 'https://www.82cook.com/entiz/enti.php?bn=15',
      contentSelectors: [
        '.board-content',
        '.board-text',
        '.content',
  ...
  async crawlPosts(): Promise<CrawledPost[]> {
    try {
      ...
      // 82쿡 게시글 목록 파싱
      const postElements = doc.querySelectorAll('tr[onclick*="view.php"], .list_item, .board_list tr')
      
      postElements.forEach((element, index) => {
        if (index >= 10) return // 최대 10개만
        
        try {
          const titleElement = element.querySelector('a[href*="view.php"]')
          const title = titleElement?.textContent?.trim() || ''
          const url = titleElement?.getAttribute('href') || ''
          const viewsElement = element.querySelector('.hit, .view_count, td:nth-child(4)')
          const views = viewsElement?.textContent?.trim() || '0'
          const timeElement = element.querySelector('.time, .date, td:nth-child(3)')
      ...
      console.error('Error crawling Cook82 posts:', error)
      return []
    }
  }
  ...
  async crawlPostContent(postUrl: string): Promise<string> {
    try {
      ...
      console.error('Error crawling Cook82 post content:', error)
